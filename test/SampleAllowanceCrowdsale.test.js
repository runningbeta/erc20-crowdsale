...
contract('SampleAllowanceCrowdsale', function ([
  ...
  it('fails to create if withdraw time is before closing time', async function () {
    await (SampleAllowanceCrowdsale.new(
      new BigNumber(6894),
      wallet,
      this.token.address,
      owner,
  ...
  describe('after opening', function () {
    ...
    it('fails to withdraw for multiple address', async function () {
      ...
      (await this.token.balanceOf(alice)).should.be.bignumber.equal(0);
      (await this.token.balanceOf(bob)).should.be.bignumber.equal(0);
    });
    ...
    it('fails to sell more than cap', async function () {
      ...
      (await this.crowdsale.balances(alice)).should.be.bignumber.equal(ether(10).mul(6894));
      (await this.crowdsale.balances(bob)).should.be.bignumber.equal(0);
    });
    ...
    it('can sell exactly until cap', async function () {
      ...
      (await this.crowdsale.balances(alice)).should.be.bignumber.equal(ether(10).mul(6894));
      (await this.crowdsale.balances(bob)).should.be.bignumber.equal(ether(5).mul(6894));
    });
  });
  ...
  describe('after withdrawal', function () {
    ...
    it('can withdraw', async function () {
      ...
      (await this.crowdsale.balances(alice)).should.be.bignumber.equal(0);
      (await this.token.balanceOf(alice)).should.be.bignumber.equal(ether(1).mul(6894));
    });

    it('fails to withdraw if not bought', async function () {
      (await this.crowdsale.balances(charlie)).should.be.bignumber.equal(0);
    ...
    it('can withdraw for multiple addresses', async function () {
      (await this.crowdsale.balances(alice)).should.be.bignumber.equal(ether(1).mul(6894));
      (await this.crowdsale.balances(bob)).should.be.bignumber.equal(ether(2).mul(6894));
      ...
      (await this.token.balanceOf(alice)).should.be.bignumber.equal(ether(1).mul(6894));
      (await this.token.balanceOf(bob)).should.be.bignumber.equal(ether(2).mul(6894));
    });
  });
});
