  ...
  describe('PublicCrowdsale', function () {
    ...
    describe('Before Crowdsale', function () {
      ...
      it('allows whitelisting of multiple addresses', async function () {
        ...
        (await this.crowdsale.getUserCap(alsoWhitelistedOne)).should.be.bignumber.equal(ether(10));
        (await this.crowdsale.getUserCap(alsoWhitelistedTwo)).should.be.bignumber.equal(ether(10));
        (await this.crowdsale.getUserCap(whitelistRevert)).should.be.bignumber.equal(ether(10));
      });

      it('allows the owner to revert whitelisted status', async function () {
        await this.crowdsale.setUserCap(whitelistRevert, ether(0));
        (await this.crowdsale.getUserCap(whitelistRevert)).should.be.bignumber.equal(ether(0));
      });
