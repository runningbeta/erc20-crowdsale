...
function shouldBehaveLikeTokenEscrow (owner, [alice, bob]) {
  ...
  describe('as a TokenEscrow', function () {
    describe('deposits', function () {
      ...
      it('can accept a single deposit', async function () {
        ...
        (await this.escrow.depositsOf(alice)).should.be.bignumber.equal(amount);
      });
      ...
      it('can accept an empty deposit', async function () {
        ...
        (await this.escrow.depositsOf(alice)).should.be.bignumber.equal(0);
      });

      it('only the owner can deposit', async function () {
        await (this.escrow.deposit(alice, 1, { from: bob })).should.be.rejectedWith(EVMRevert);
      ...
      it('can track deposits to multiple accounts', async function () {
        ...
        (await this.escrow.depositsOf(alice)).should.be.bignumber.equal(amount.div(3));
        (await this.escrow.depositsOf(bob)).should.be.bignumber.equal(amount.div(3).mul(2));
      });
    });
    ...
    describe('withdrawals', async function () {
      it('can withdraw payments', async function () {
        ...
        const payeeFinalBalance = await this.token.balanceOf(alice);
      ...
      it('only the owner can withdraw', async function () {
        await (this.escrow.withdraw(alice, { from: alice })).should.be.rejectedWith(EVMRevert);
      });
